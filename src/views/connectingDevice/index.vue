...
onMounted(() => {
  // getfl().then((res) => {
  //   console.log("��� ~ getfltotal ~ res:", res)
  //   num.value = res
  // })
  // getfltotal().then((res) => {
  //   console.log("��� ~ getfltotal ~ res:", res)
...
const handleUpdate = (row: any) => {
  getTableDataApiId({ id: row.id }).then((res: any) => {
    tableData2.value = res.deviceChannels
...
const getTableData = () => {
  ...
  getTableDataApi({
    ...
    EquipmentTypeName: searchData.MonitorProductTypeName || undefined,
    Manufacturer: currentS.value === "全部" ? null : currentS.value
  })
    .then((data) => {
...
console.log("🚀 ~ getTableData ~ getTableData:", getTableData)
console.log("🚀 ~ getTableData ~ getTableData:", getTableData)
const handleSearch = () => {
...
const historyHandleUpdate = () => {
  loding1.value = true
  getTableDataApiIdHistory({
    ManufactureDeviceId: historyDialogId.value,
...
<template>
  ...
  <el-dialog v-model="dialogVisible" title="设备详情" width="70%">
    ...
    <div class="flex-1 ml-4 rounded-1 flex flex-col">
      <div class="bg-white h-14 rounded-1 flex items-center pl-4">
        <el-form ref="searchFormRef" :inline="true" :model="searchData" class="flex w-full items-center">
          ...
          <el-form-item prop="MonitorProductTypeName" label="设备类型">
  ...
  <!-- <el-card v-loading="loading" class="mx-10 mt-8">
    <el-form ref="searchFormRef" :inline="true" :model="searchData" class="flex w-full items-center">
      ...
      <el-form-item prop="MonitorProductTypeName" label="设备状态">
        <el-input v-model="searchData.MonitorProductTypeName" placeholder="请输入" />
      </el-form-item>
